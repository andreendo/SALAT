 ...
 */
class TesterAltered {
    private final static Logger LOGGER = Logger.getLogger(TesterAltered.class.getName());
    private final DriverAltered driver;
    private final StopCondition stopCondition;
    private final Random random;

    public TesterAltered(DriverAltered driver, StopCondition stopCondition, Random random) {
        this.driver = driver;
        ...
        while(! stopCondition.hasReached()) {
            ...
            if(events.isEmpty()) { 
                ...
                if(driver.execute( events.get(randIndex) )) {
                    ...
                    if(driver.isOut()) {
                        LOGGER.info("Reach a state outside of app under test. Going back...");
